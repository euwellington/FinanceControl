...
const AddTransaction = () => {
  const { categoryStore, peopleStore } = useContext(StoresContext);
  const { insert, loading } = useTransaction();
  ...
  }, [isOpen, categoryStore, peopleStore]);

  const handleOpen = () => {
    setRequest({ type: "Expense" });
    setErrors({});
    setIsOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validationErrors = TransactionValidation(
      request
    );
    ...
    await insert(request as ITransaction, () => setIsOpen(false));
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <FieldGroup className="py-4 space-y-4">
            <Field>
              ...
              <Input
                id="description"
                placeholder="Ex: Compra de materiais, Freelance..."
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button type="submit" className="bg-[#2c2f6e] ml-2 text-white hover:bg-[#3a3e8f]" disabled={loading}>
              {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Salvar Transação"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
